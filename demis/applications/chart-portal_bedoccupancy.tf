...
module "portal_bedoccupancy" {
  ...
  application_values = templatefile(local.portal_bedoccupancy_template_app, {
    ...
    istio_proxy_resources                              = try(local.portal_bedoccupancy_resources_overrides.istio_proxy_resources, var.istio_proxy_default_resources)
  })
  istio_values = templatefile(local.portal_bedoccupancy_template_istio, {
    ...
    istio_rules_block_external = try(module.external_routing_configurations[0].rules[local.portal_bedoccupancy_name], [])
  })
}
