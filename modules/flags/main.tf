locals {
  ...
  service_config_options = { for s in distinct(flatten([for co in var.config_options : co.services])) : s => { for co in var.config_options : co.option_name => co.option_value if contains(co.services, s) } }
}
