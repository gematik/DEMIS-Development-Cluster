...
resource "terraform_data" "main_profiles" {
  input = distinct(compact(length(var.deployment_information.main.profiles) > 0 ? var.deployment_information.main.profiles : [var.default_profile_snapshots]))
}

resource "terraform_data" "subsets_main" {
  input = toset(yamldecode(templatefile("${path.module}/.scripts/subset.tftpl.yaml", {
